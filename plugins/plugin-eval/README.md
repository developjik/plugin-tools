...
- a Claude Code plugin bundle in this dual-platform packaging
...
- `bin/plugin-eval`: a Claude Code-friendly local wrapper for the CLI
- `skills/`: the plugin's chat-facing skills

The plugin is designed to feel chat-first in Codex and Claude Code, while still routing to explicit local commands you can run yourself.

## Source

This directory packages the public [`thisdot/plugin-eval`](https://github.com/thisdot/plugin-eval) project as a root-level dual-platform plugin bundle.
...
## How It Works As A Codex Or Claude Code Plugin

This directory is also a plugin bundle. The Codex manifest lives at [`.codex-plugin/plugin.json`](./.codex-plugin/plugin.json), the Claude Code manifest lives at [`.claude-plugin/plugin.json`](./.claude-plugin/plugin.json), and both expose the shared skills under [`skills/`](./skills).
...
In Claude Code, the plugin also ships [`bin/plugin-eval`](./bin/plugin-eval), which delegates to the local Node CLI so plugin skills can call `plugin-eval` without requiring a global `npm link`.

## Manual Plugin Installation

Codex plugin discovery is marketplace-based. This repository root is the plugin folder because it contains `.codex-plugin/plugin.json`, and Codex discovers it through a `marketplace.json` file.

The key path rule is:

- in this repository's `.agents/plugins/marketplace.json`, `.` resolves to this repository root
- in `~/.agents/plugins/marketplace.json`, `./plugins/plugin-eval` resolves to `~/plugins/plugin-eval`
- in another workspace's `.agents/plugins/marketplace.json`, `./plugins/plugin-eval` resolves to `<workspace>/plugins/plugin-eval`
...
ln -sfn /absolute/path/to/this/repo ~/plugins/plugin-eval
```
...
ln -sfn /absolute/path/to/this/repo /path/to/workspace/plugins/plugin-eval
```
...
This repository already includes a workspace marketplace entry for `plugin-eval` in [`.agents/plugins/marketplace.json`](./.agents/plugins/marketplace.json). It uses `.` because the plugin bundle now lives at the repository root.
