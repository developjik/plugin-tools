export function buildImprovementBrief(result) {
  ...
  const isClean = prioritized.length === 0 && result.summary.topRecommendations.length === 0;
  const measurementGoals =
    !isClean && result.measurementPlan?.recommendedToolsets?.length > 0
  ...
  if (isClean) {
    return {
      ...
      summary: `No structural, budget, or best-practice fixes are currently recommended for ${result.target.name}.`,
      goals: [],
      measurementGoals: [],
      requiredFixes: [],
      recommendedFixes: [],
      suggestedPrompt: `No rewrite is needed for ${result.target.name} based on the current evaluation.`,
    };
  }
